...
export default function ContentManager({ resource, initialRows }) {
  ...
  return (
    <>
      ...
      {rows.length === 0 ? <div className="panel empty-state"><span className="empty-icon" aria-hidden="true">◇</span><h2>Your canvas is clear</h2><p>No {config.label.toLowerCase()} yet. Add the first one when you are ready.</p></div> : <div className="content-list">{rows.map((row,index) => <article className="panel content-row" key={row.id}><div className="row-copy"><span className="row-index">{String(index + 1).padStart(2,"0")}</span><div><h2>{row.title || row.name || row.position || row.degree || row.label}</h2><p className="muted">{row.company || row.issuer || row.institution || row.organization || row.summary || "Ready to edit"}</p></div></div><div className="row-actions"><span className={row.visible ? "pill visible" : "pill"}><i aria-hidden="true"/>{row.visible ? "Live" : "Hidden"}</span><button aria-label="Move up" disabled={index === 0} onClick={() => move(index,-1)}>↑</button><button aria-label="Move down" disabled={index === rows.length - 1} onClick={() => move(index,1)}>↓</button><button onClick={() => start(row)}>Edit</button><button className="danger" onClick={() => remove(row.id)}>Delete</button></div></article>)}</div>}
