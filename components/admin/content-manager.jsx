...
export default function ContentManager({ resource, initialRows, categoryOptions = [], enabledSections = {} }) {
  ...
  const sectionEnabled = enabledSections[resourceSections[resource]] !== false;
  ...
  async function toggleVisibility(row) {
    if (!sectionEnabled || savingVisibility !== null) return;
    const visible = !Boolean(row.visible);
    setSavingVisibility(row.id);
    setMessage("");
    try {
      const response = await fetch(`/api/admin/${resource}/${row.id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ visible }),
      });
      const result = await response.json();
      if (!response.ok) throw new Error(result.error || "Could not update visibility.");
      setRows((current) => current.map((item) => item.id === row.id ? { ...item,visible } : item));
      setError(false);
      setMessage(`${config.singular[0].toUpperCase()}${config.singular.slice(1)} ${visible ? "shown" : "hidden"}.`);
      router.refresh();
    } catch (caught) {
      setError(true);
      setMessage(caught instanceof Error ? caught.message : "Could not update visibility.");
      router.refresh();
    } finally {
  ...
  return (
    <>
          ...
          <section className="modal-card" role="dialog" aria-modal="true" aria-labelledby="editor-title">
            ...
            <form className="editor-form" onSubmit={save}>
              {config.fields.map(([name,label,type="text",required,options]) => {
                ...
                if (type === "checkbox") return <label className="check-field" key={name}><span>{label}</span><input className="switch-input" role="switch" type="checkbox" checked={Boolean(form[name])} onChange={(event) => setForm({ ...form, [name]: event.target.checked })}/></label>;
      ...
      {rows.length === 0 ? <div className="panel empty-state"><span className="empty-icon" aria-hidden="true">◇</span><h2>Your canvas is clear</h2><p>No {config.label.toLowerCase()} yet. Add the first one when you are ready.</p></div> : <div className="content-list">{rows.map((row,index) => { const category=resource === "skills" ? categoryOptions.find((item) => Number(item.id) === Number(row.category_id)) : null;const label=row.title || row.name || row.position || row.degree || row.label;return <article className="panel content-row" key={row.id}><div className="row-copy"><span className="row-index">{String(index + 1).padStart(2,"0")}</span><div><h2>{label}</h2><p className="muted">{category?.name || row.company || row.issuer || row.institution || row.organization || row.summary || (resource === "skills" ? "Uncategorised" : "Ready to edit")}</p></div></div><div className="row-actions"><label className="row-visibility" title={sectionEnabled ? `Show or hide ${label}` : "Enable this section in Settings first"}><span>{sectionEnabled ? (row.visible ? "Visible" : "Hidden") : "Section off"}</span><input className="switch-input" role="switch" type="checkbox" checked={Boolean(row.visible)} disabled={!sectionEnabled || savingVisibility !== null} onChange={() => toggleVisibility(row)} aria-label={`Show ${label} on portfolio`}/></label><button aria-label="Move up" title="Move up" disabled={index === 0} onClick={() => move(index,-1)}><Icon name="up"/></button><button aria-label="Move down" title="Move down" disabled={index === rows.length - 1} onClick={() => move(index,1)}><Icon name="down"/></button><button aria-label={`Edit ${label}`} title="Edit" onClick={() => start(row)}><Icon name="edit"/></button><button className="danger" aria-label={`Delete ${label}`} title="Delete" onClick={() => remove(row.id)}><Icon name="delete"/></button></div></article>})}</div>}
