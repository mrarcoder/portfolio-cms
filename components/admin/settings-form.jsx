...
export default function SettingsForm({ initial }) {
  ...
  return <>
    <form className="panel editor-form settings-editor" onSubmit={save}>
      ...
      <section className="form-section"><div className="form-section-heading"><span><Icon name="eye"/></span><div><h2>Public sections</h2><p>Control which content areas appear in the navigation and portfolio.</p></div></div><div className="section-toggle-grid">{sectionNames.map((name) => <label className="section-toggle" key={name}><span>{name[0].toUpperCase() + name.slice(1)}</span><input className="switch-input" role="switch" type="checkbox" checked={form.enabled_sections?.[name] !== false} onChange={(event) => setForm({ ...form,enabled_sections:{ ...form.enabled_sections,[name]:event.target.checked } })}/></label>)}</div></section>
