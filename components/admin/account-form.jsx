...
export default function AccountForm({ initialUsername }) {
  ...
  return <><section className="settings-section"><div><p className="eyebrow">Administrator account</p><h2>Login credentials</h2><p className="muted">Confirm your current password before changing the username or password.</p></div><form className="panel editor-form settings-editor" onSubmit={save}><label className="field"><span>Username</span><input value={username} onChange={(event) => setUsername(event.target.value)} autoComplete="username" required minLength="3" maxLength="40" pattern="[A-Za-z0-9_-]+"/><small>3–40 letters, numbers, hyphens, or underscores.</small></label><label className="field"><span>Current password</span><input type="password" value={currentPassword} onChange={(event) => setCurrentPassword(event.target.value)} autoComplete="current-password" required minLength="12" maxLength="128"/></label><label className="field"><span>New password</span><input type="password" value={newPassword} onChange={(event) => setNewPassword(event.target.value)} autoComplete="new-password" minLength="12" maxLength="128"/><small>Leave blank to keep the current password.</small></label><label className="field"><span>Confirm new password</span><input type="password" value={confirmPassword} onChange={(event) => setConfirmPassword(event.target.value)} autoComplete="new-password" required={Boolean(newPassword)} minLength="12" maxLength="128"/></label><div className="form-actions"><button className="button icon-only no-margin" disabled={busy} aria-label={busy ? "Updating credentials" : "Update credentials"} title="Update credentials"><Icon name={busy ? "reset" : "apply"} className={busy ? "icon-spinning" : ""}/></button></div></form></section><Toast message={message} clear={setMessage} error={error}/></>;
}
