...
export default function ProfilePhotoEditor({ currentMediaId, file, onFileChange }) {
  ...
  return <><div className="profile-photo-field">
    ...
    <div className="profile-photo-card">
      ...
      <div className="profile-photo-copy">
        ...
        <div className="profile-photo-actions">
          {sourceUrl && <button className="secondary-button" type="button" onClick={() => setMode("view")}>View</button>}
          {sourceUrl && <button className="secondary-button" type="button" onClick={openEditor}>Edit</button>}
          <button className="secondary-button" type="button" onClick={() => inputRef.current?.click()}>{sourceUrl ? "Upload new" : "Upload photo"}</button>
        </div>
      </div>
    </div>
    ...
    {mode === "view" && <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setMode(null); }}><div className="modal-card photo-viewer" role="dialog" aria-modal="true" aria-labelledby="photo-view-title"><div className="modal-head"><div><p className="eyebrow">Profile photo</p><h2 id="photo-view-title">Current preview</h2></div><button className="icon-button" type="button" onClick={() => setMode(null)} aria-label="Close photo viewer" autoFocus>×</button></div><Image src={sourceUrl} width={800} height={1000} unoptimized alt="Full profile photo preview"/></div></div>}

    {mode === "edit" && <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setMode(null); }}><div className="modal-card photo-editor-modal" role="dialog" aria-modal="true" aria-labelledby="photo-editor-title"><div className="modal-head"><div><p className="eyebrow">Profile photo</p><h2 id="photo-editor-title">Crop and adjust</h2></div><button className="icon-button" type="button" onClick={() => setMode(null)} aria-label="Close photo editor">×</button></div><div className="photo-editor-layout"><div className="photo-crop-stage">{!image && !error && <span>Loading photo…</span>}<canvas ref={canvasRef} width="640" height="800" aria-label="Edited photo preview"/></div><div className="photo-adjustments"><RangeControl label="Zoom" value={adjustments.zoom} min={1} max={3} step={.05} suffix="×" onChange={(value) => update("zoom",value)}/><RangeControl label="Horizontal" value={adjustments.x} min={0} max={100} suffix="%" onChange={(value) => update("x",value)}/><RangeControl label="Vertical" value={adjustments.y} min={0} max={100} suffix="%" onChange={(value) => update("y",value)}/><RangeControl label="Brightness" value={adjustments.brightness} min={70} max={130} suffix="%" onChange={(value) => update("brightness",value)}/><RangeControl label="Contrast" value={adjustments.contrast} min={70} max={130} suffix="%" onChange={(value) => update("contrast",value)}/><div className="photo-rotate"><span>Rotate</span><div><button className="secondary-button" type="button" onClick={() => update("rotation",adjustments.rotation - 90)} aria-label="Rotate left">↶ Left</button><button className="secondary-button" type="button" onClick={() => update("rotation",adjustments.rotation + 90)} aria-label="Rotate right">Right ↷</button></div></div></div></div>{error && <p className="photo-editor-error" role="alert">{error}</p>}<div className="photo-editor-actions"><button className="secondary-button" type="button" onClick={() => setAdjustments(DEFAULT_ADJUSTMENTS)}>Reset</button><button className="button no-margin" type="button" disabled={!image || processing} onClick={applyEdit}>{processing ? "Applying…" : "Apply changes"}</button></div></div></div>}
  </>,document.body)}</>;
}
