...
export default function Portfolio({ data }) {
  ...
  return <div className="portfolio-shell" style={{ "--accent":s.primary_color || "#70f0c0" }} data-mode={s.color_mode || "dark"}>
    ...
    <header className="public-header"><div className="container header-inner"><a className="brand" href="#top"><span className="brand-mark">{initials}</span><span>{p.name || s.site_name || "Portfolio"}</span></a><nav className="public-nav" aria-label="Portfolio sections">{navigation.map(([id,label]) => <a className={id === "contact" ? "nav-cta" : undefined} href={`#${id}`} key={id}>{label}{id === "contact" && <span aria-hidden="true"> ↗</span>}</a>)}</nav></div></header>
    <main id="main">
      <section className="public-hero container" id="top">
        <div className="hero-copy">{p.location && <p className="eyebrow">Based in {p.location}</p>}<h1><span className="hero-name">{p.name || "Your name"}</span><span className="hero-title">{p.title || "Your professional title"}</span></h1><p className="lede">{p.short_bio || s.site_description || "Add your introduction from the admin panel."}</p><div className="hero-actions">{enabled.contact !== false && <a className="button no-margin" href="#contact">Start a conversation <span aria-hidden="true">↗</span></a>}{p.resume_media_id && <a className="secondary-link" href={`/api/media/${p.resume_media_id}`}>Download résumé <span aria-hidden="true">↓</span></a>}</div></div>
        <div className="portrait-frame"><div className="orbit orbit-one"/><div className="orbit orbit-two"/>{p.photo_media_id ? <Image className="portrait" src={`/api/media/${p.photo_media_id}`} width={520} height={620} alt={`${p.name} portrait`} priority unoptimized/> : <div className="portrait-placeholder"><span>{initials}</span><small>Portrait</small></div>}</div>
      </section>
      <div className="container">
        {sections.about && <Section id="about" title="About"><h2 className="section-title">{p.title || "About me"}</h2><p className="section-copy pre-line">{p.long_bio}</p></Section>}
        {sections.experience && <Section id="experience" title="Experience"><div className="timeline">{data.experiences.map((item,index) => <article key={item.id}><span className="timeline-index">{String(index+1).padStart(2,"0")}</span><div><h2>{item.position}</h2><p>{item.company}{item.location && ` · ${item.location}`}</p></div><div><p className="muted"><Dates start={item.start_date} end={item.end_date} current={item.is_current}/></p><p className="pre-line">{item.description}</p></div></article>)}</div></Section>}
        {sections.projects && <Section id="projects" title="Selected work"><h2 className="section-title">Ideas made tangible.</h2><div className="project-grid">{data.projects.map((item,index) => <article className="project-card" key={item.id}>{item.image_media_id ? <Image src={`/api/media/${item.image_media_id}`} width={720} height={450} alt={item.title} unoptimized/> : <div className="project-placeholder"><span>{String(index+1).padStart(2,"0")}</span></div>}<div><div className="card-meta"><p>{item.technologies.join(" · ")}</p><span aria-hidden="true">↗</span></div><h2><Link href={`/projects/${item.slug}`}>{item.title}</Link></h2><p>{item.summary}</p></div></article>)}</div></Section>}
        {sections.skills && <Section id="skills" title="Capabilities"><div className="skill-groups">{data.skillCategories.map((category) => <article key={category.id}><span className="skill-glow"/><h2>{category.name}</h2><p>{data.skills.filter((item) => item.category_id === category.id).map((item) => item.name).join(" · ")}</p></article>)}</div></Section>}
        {sections.education && <Section id="education" title="Education"><div className="simple-grid">{data.education.map((item) => <article key={item.id}><p className="eyebrow"><Dates start={item.start_date} end={item.end_date}/></p><h2>{item.degree}</h2><p>{item.institution}{item.field && ` · ${item.field}`}</p></article>)}</div></Section>}
        {sections.achievements && <Section id="achievements" title="Achievements"><div className="simple-grid">{data.achievements.map((item) => <article key={item.id}><p className="eyebrow">{item.date}</p><h2>{item.title}</h2><p>{item.description}</p><p className="muted">{item.organization}</p></article>)}</div></Section>}
        {sections.certifications && <Section id="certifications" title="Certifications"><div className="simple-grid">{data.certifications.map((item) => <article key={item.id}><p className="eyebrow">Issued {item.issue_date}</p><h2>{item.name}</h2><p>{item.issuer}</p><div className="credential-links">{item.credential_url && <a href={item.credential_url}>View credential ↗</a>}{item.file_media_id && <a href={`/api/media/${item.file_media_id}`}>Download certificate ↓</a>}</div></article>)}</div></Section>}
